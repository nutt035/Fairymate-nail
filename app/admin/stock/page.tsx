...
export default function StockManagement() {
    ...
    // Fetch stock items from inventory table
    ...
    const getStockStatus = (item: StockItem) => {
        ...
        if (item.quantity <= item.min_level) return { label: 'ใกล้หมด', color: 'bg-amber-100 text-amber-600' };
    ...
    // Count stats
    const lowStockCount = stockItems.filter(i => i.quantity <= i.min_level && i.quantity > 0).length;
    const outOfStockCount = stockItems.filter(i => i.quantity === 0).length;
    const totalValue = stockItems.reduce((sum, i) => sum + (i.quantity * (i.cost_per_unit || 0)), 0);
                            ...
                            >
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-slate-400">{item.category || 'อื่นๆ'}</p>
                                    </div>
                                ...
                                <div className="flex items-end justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400 mt-1">
                                            ต้นทุน: {formatCurrency(item.cost_per_unit || 0)}/{item.unit}
                                        </p>
                                    </div>
