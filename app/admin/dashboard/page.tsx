...
export default function Dashboard() {
  // Queue + Income States (จาก bookings)
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // 1) Today's bookings (คิววันนี้ — ยังไม่รวมที่ยกเลิก)
      ...
      // 2) Today income (งานที่จ่ายเงินแล้ววันนี้)
      ...
      const todayTotal =
        todayBookings?.reduce((sum, r) => sum + (r.final_price || 0), 0) || 0;
      ...
      const { data: monthBookings } = await supabase
        ...
        .lte("booking_date", endMonth);

      const monthTotal =
        monthBookings?.reduce((sum, r) => sum + (r.final_price || 0), 0) || 0;
      setMonthlyIncome(monthTotal);

      // 4) Goal (จาก shop_settings แทนตาราง goals)
      ...
      if (settings && settings.monthly_target) {
        setGoalAmount(Number(settings.monthly_target));
      }
    } catch (error) {
  ...
  const fetchProfitData = async () => {
    try {
      ...
      // 3) Weekly data for trend chart (รายได้จาก bookings ที่จ่ายแล้ว)
  ...
  const handleSaveGoal = async () => {
    ...
    try {
      await supabase
        .from("shop_settings")
        .upsert({ id: 1, monthly_target: newAmount }, { onConflict: "id" });
          ...
          <div className="bg-white rounded-[20px] p-4 md:p-6 shadow-soft min-h-[400px] md:min-h-[500px]">
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                queues.map((q, idx) => {
                  const isNow = idx === 0 && q.status !== "done" && q.status !== "cancelled";
                  return (
                    <div
                      key={q.id}
              ...
              <div className="flex justify-between items-start">
                ...
                {!isEditingGoal ? (
                  <button
                    ...
                    className="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-white/80 hover:text-white"
                  >
                    <Edit2 size={14} />
                  </button>
                ) : (
